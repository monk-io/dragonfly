...
Batch mode (--batch <file> | -b <file>):
  ...
  Still honored from the environment with --batch (they are global, not per-run):
    SERVER_HOST, CLIENT_HOST, SSH_USER  - cross-machine topology, applied to every run.
...
Examples:
  ...
  SERVER_HOST=172.31.30.209 CLIENT_HOST=172.31.20.3 ./bench_v2.sh --batch stage1.bench
