...
void CompactObj::SetCuckooFilter(uint64_t capacity, uint8_t slots_per_bucket,
  ...
  // Allocate before touching taglen_/u_: if construction throws (e.g. bad_alloc from an
  // oversized capacity), this object must stay in its previous, valid state. Setting the tag
  // first would leave a CUCKOO_FILTER_TAG object with a stale/garbage u_.cuckoo_filter pointer.
  CuckooFilter* cf =
